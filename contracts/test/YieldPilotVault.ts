...
describe("YieldPilotVault", async function () {
	...
	it("deploys through an Ignition transparent proxy and unwinds strategy liquidity on withdraw", async function () {
		const asset = await viem.deployContract("MockERC20", [
			"Mock USD Coin",
			"mUSDC",
			6,
		]);
		...
		const strategyOne = await viem.deployContract("MockStrategyAdapter", [
			asset.address,
		]);
		const strategyTwo = await viem.deployContract("MockStrategyAdapter", [
			asset.address,
		]);
		...
		await vault.write.deployToStrategy([
			strategyOne.address,
			parseUnits("11000", 6),
		]);
		await vault.write.deployToStrategy([
		...
		assert.deepEqual(
			(await vault.read.activeStrategies()).map((address) =>
				address.toLowerCase(),
			),
			[strategyOne.address, strategyTwo.address].map((address) =>
				address.toLowerCase(),
			),
		);
		assert.equal(
			await vault.read.totalStrategyAssets(),
			parseUnits("20000", 6),
		);
		...
		await vault.write.withdraw(
			[withdrawAmount, alice.account.address, alice.account.address],
			{
				account: alice.account,
			},
		);
	...
	it("allows the owner to deposit and deploy to a whitelisted strategy on behalf of a user", async function () {
		const asset = await viem.deployContract("MockERC20", [
			"Mock USD Coin",
			"mUSDC",
			6,
		]);
		const depositAmount = parseUnits("7500", 6);

		await asset.write.mint([alice.account.address, depositAmount]);

		const { vault } = await ignition.deploy(YieldPilotVaultModule, {
			deploymentId: "vault-deposit-and-deploy-for",
			parameters: {
				YieldPilotVaultProxyModule: {
					asset: asset.address,
					name: "Kabon USDC Vault",
		...
		const previewShares = await vault.read.previewDeposit([depositAmount]);

		await vault.write.depositAndDeployToStrategyFor([
		...
		assert.equal(await vault.read.idleAssets(), 0n);
		assert.equal(await vault.read.totalStrategyAssets(), depositAmount);
		assert.equal(await vault.read.totalAssets(), depositAmount);
		assert.equal(await strategy.read.totalAssets(), depositAmount);
	});

	it("rejects deposit-and-deploy-for when the strategy is not whitelisted", async function () {
		const asset = await viem.deployContract("MockERC20", [
			"Mock USD Coin",
			"mUSDC",
			6,
		...
		const { vault } = await ignition.deploy(YieldPilotVaultModule, {
			deploymentId: "vault-deposit-and-deploy-for-unlisted-strategy",
			parameters: {
				YieldPilotVaultProxyModule: {
					asset: asset.address,
					name: "Kabon USDC Vault",
		...
		await assert.rejects(
			vault.write.depositAndDeployToStrategyFor([
				alice.account.address,
				alice.account.address,
				strategy.address,
				depositAmount,
	...
	it("supports strategy accounting sync and blocks fresh deposits while paused", async function () {
		...
		await vault.write.withdraw(
			[parseUnits("500", 6), alice.account.address, alice.account.address],
			{
				account: alice.account,
			},
		);
	...
	it("requires pausing before syncing realized strategy losses", async function () {
		...
		await strategy.write.sweepLoss([
			owner.account.address,
			parseUnits("500", 6),
		]);

		await assert.rejects(vault.write.syncStrategyAssets([strategy.address]));
	...
	it("uses the configured withdrawal queue order during unwind", async function () {
		...
		await vault.write.setWithdrawalQueue([
			[strategyTwo.address, strategyOne.address],
		]);

		assert.deepEqual(
			(await vault.read.withdrawalQueue()).map((address) =>
				address.toLowerCase(),
			),
			[strategyTwo.address, strategyOne.address].map((address) =>
				address.toLowerCase(),
			),
		);

		await vault.write.withdraw(
			[parseUnits("5000", 6), alice.account.address, alice.account.address],
			{
				account: alice.account,
			},
		);
	...
	it("charges the unwind fee only on the recalled portion of a withdrawal", async function () {
		...
		const [availableNow, needsUnwind, feeAssets] =
			await vault.read.previewWithdrawalFee([withdrawAmount]);
		...
		assert.equal(
			await vault.read.previewRedeem([previewShares]),
			withdrawAmount,
		);
	...
	it("reduces redeem output by the unwind fee while keeping instant redemptions fee-free", async function () {
		...
		assert.equal(
			await vault.read.previewRedeem([parseUnits("2000", 6)]),
			parseUnits("2000", 6),
		);
		assert.equal(
			await vault.read.previewRedeem([parseUnits("5009", 6)]),
			parseUnits("5000", 6),
		);
		...
		await vault.write.redeem(
			[parseUnits("5009", 6), alice.account.address, alice.account.address],
			{
				account: alice.account,
			},
		);
	...
	it("blocks strategy callback reentry during deployment", async function () {
		...
		const strategy = await viem.deployContract("ReentrantStrategyAdapter", [
			asset.address,
			owner.account.address,
		]);
		await strategy.write.setVault([vault.address]);
		await strategy.write.setReentry([
			parseUnits("1000", 6),
			owner.account.address,
		]);
		await vault.write.whitelistStrategy([strategy.address]);
	...
	it("blocks strategy callback reentry during withdrawal unwinds", async function () {
		...
		await strategy.write.setRedeemReentry([
			parseUnits("1", 6),
			alice.account.address,
		]);
		...
		await assert.rejects(
			vault.write.withdraw(
				[parseUnits("2500", 6), alice.account.address, alice.account.address],
				{
					account: alice.account,
				},
			),
	...
	it("upgrades through the transparent proxy admin and preserves vault state", async function () {
		...
		const { vault, proxy, proxyAdmin } = await ignition.deploy(
			YieldPilotVaultModule,
			{
		...
		const implementationV2 = await viem.deployContract(
			"YieldPilotVaultUpgradeMock",
		);
		...
		await proxyAdmin.write.upgradeAndCall(
			[proxy.address, implementationV2.address, initializeV2Data],
			{
				account: owner.account,
			},
		);

		const upgradedVault = await viem.getContractAt(
			"YieldPilotVaultUpgradeMock",
			proxy.address,
		);
		...
		assert.equal(
			await upgradedVault.read.balanceOf([alice.account.address]),
			depositAmount,
		);
	});
	...
	it("supports the dedicated Ignition upgrade module from the Hardhat guide pattern", async function () {
		...
		const { upgradedVault } = await ignition.deploy(
			UpgradeYieldPilotVaultModule,
			{
	...
	it("lets the owner configure the unwind fee within the capped range", async function () {
		...
		assert.equal(
			(await vault.read.feeRecipient()).toLowerCase(),
	...
	it("rejects fee-on-transfer assets during deposit accounting", async function () {
		const asset = await viem.deployContract("FeeOnTransferERC20", [
			"Fee USD Coin",
			"fUSDC",
			6,
			100,
	...
	it("rejects strategies that misreport deployed assets", async function () {
		...
		const strategy = await viem.deployContract("MisreportingStrategyAdapter", [
			asset.address,
		]);
		await strategy.write.setVault([vault.address]);
