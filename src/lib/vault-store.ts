...
type VaultState = {
  ...
  walletUsdc: number;
  vaultBalance: number;
  policy: { risk: RiskPreset; liquidity: LiquidityPreset; perStrategyMax: number } | null;
  ...
  withdraw(amount: number) {
    set((s) => {
      ...
      let newAllocs = s.allocations.map((a) =>
        a.strategyId === "idle"
          ? { ...a, percent: newVault > 0 ? (idleUsd - fromIdle) / newVault : 0 }
          : { ...a, percent: newVault > 0 ? (a.percent * s.vaultBalance) / newVault : 0 },
      );
      // normalize
      const total = newAllocs.reduce((a, b) => a + b.percent, 0);
      if (total > 0) newAllocs = newAllocs.map((a) => ({ ...a, percent: a.percent / total }));
