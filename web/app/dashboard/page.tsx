...
type StoredCopilotState = {
  result: CopilotAnalysisResponse | null;
  signals: Record<string, CopilotSignal>;
  updatedAt: number;
};
...
export default function DashboardPage() {
  ...
  const portfolioCopilotResult =
    copilotResult?.mode === "portfolio" ? copilotResult : null;
  const portfolioCopilotRecommendation =
    portfolioCopilotResult?.bestOpportunity ?? null;
  const bestCopilotAsset = useMemo(
    () =>
      copilotResult?.bestOpportunity
        ? (assets.find(
            (asset) => asset.id === copilotResult.bestOpportunity?.assetId,
          ) ?? null)
        : null,
  ...
  const copilotReadyCount = useMemo(
    () =>
      Object.values(copilotSignals).filter(
        (signal) => signal.state === "actionable",
      ).length,
    [copilotSignals],
  );
  const copilotStorageKey = `${COPILOT_STORAGE_PREFIX}:${address?.toLowerCase() ?? "disconnected"}`;
  const walletAssetIdKey = useMemo(
    () =>
  ...
  function applyCopilotResult(result: CopilotAnalysisResponse) {
    ...
    setCopilotSignals((current) => {
      ...
      persistCopilotState({
        result:
          result.mode === "portfolio" ? result : (copilotResult ?? result),
        signals: next,
        updatedAt: Date.now(),
      });
      return next;
  ...
  function persistCopilotState(nextState: StoredCopilotState) {
    if (typeof window === "undefined") return;
    window.sessionStorage.setItem(copilotStorageKey, JSON.stringify(nextState));
  }

  useEffect(() => {
    if (typeof window === "undefined") return;
    if (walletAssetIdKey.length === 0) return;

    const stored = window.sessionStorage.getItem(copilotStorageKey);
    if (!stored) return;

    try {
      const parsed = JSON.parse(stored) as Partial<StoredCopilotState>;
      const currentAssetIds = new Set(walletAssetIdKey.split("|"));
      const restoredSignals = Object.fromEntries(
        Object.entries(parsed.signals ?? {}).filter(([assetId]) =>
      ...
      setCopilotSignals(restoredSignals);
      setCopilotResult(restoredResult);
    } catch {
  ...
  return (
    <div className="px-0 pb-12">
        ...
        <div className="flex flex-col gap-8">
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
                      {portfolioCopilotResult
                        ? portfolioCopilotResult.headline
                        : walletAssetsWithMarketData.length > 0
                          ? `${supportedWalletAssets} deposit-ready assets found.`
                          : "Connect a wallet to scan routes."}
                    </p>
                    <p className="mt-1 text-xs leading-relaxed text-muted-foreground/70">
                      Ranks approved vault routes without moving funds.
                    </p>
                  </div>
                  <Button
                    type="button"
                  ...
                  >
                    ...
                    {portfolioCopilotResult ? "Refresh" : "Analyze"}
                  </Button>
                </div>
              </div>
            </div>
          ...
          <PositionsCard
            copilotRecommendation={
              portfolioCopilotRecommendation
                ? {
                    label: portfolioCopilotRecommendation.decisionTitle,
                    summary: portfolioCopilotRecommendation.summary,
                    onReview: () => void openCopilot({ mode: "portfolio" }),
                  }
                : undefined
            }
          />
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {portfolioCopilotResult ? (
                    <div className="rounded-full border border-emerald-500/25 bg-emerald-500/10 px-3 py-1.5 text-xs font-medium text-emerald-300">
                      {copilotReadyCount} AI-ranked asset
                      {copilotReadyCount === 1 ? "" : "s"}
                    </div>
                  ) : null}
                </div>
                <Button
      ...
      <Sheet open={copilotOpen} onOpenChange={setCopilotOpen}>
        <SheetContent
          side="right"
          className="w-full border-border/80 bg-popover p-0 sm:max-w-[680px]"
        >
                  ...
                  <div className="border-b border-border/70 pb-6">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="inline-flex rounded-md border border-border/60 px-2 py-0.5 text-[11px] font-semibold text-foreground/90">
                          {copilotResult.bestOpportunity
                            ? `${copilotResult.bestOpportunity.expectedApy.toFixed(2)}% APY`
                            : "Hold"}
                        </span>
                      </div>
                      <div className="mt-3 max-w-[520px] text-2xl font-semibold tracking-tight text-foreground">
                    ...
                    <div className="mt-6 space-y-5">
                      {copilotResult.drivers.length > 0 ? (
                        <div>
                          ...
                          <ul className="mt-2 space-y-2">
                            {copilotResult.drivers.slice(0, 3).map((driver) => (
                              <li
                                key={driver}
                      ...
                      {copilotResult.cautions.length > 0 ? (
                        <div>
                          <div className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground/70">
                            Risks
                          </div>
                          <ul className="mt-2 space-y-2">
                            {copilotResult.cautions
                              .slice(0, 2)
                              .map((caution) => (
                                <li
                                  key={caution}
                    ...
                    <div className="mt-6 border-t border-border/60 pt-5">
                      ...
                      <p className="mt-2 max-w-[560px] text-sm leading-relaxed text-foreground">
                        {copilotResult.nextAction}
                      </p>
                      <div className="mt-4 grid grid-cols-2 gap-2">
                        <Button
                          ...
                          disabled={!bestCopilotAsset?.supported}
                          onClick={() => {
                            if (!bestCopilotAsset?.supported) return;
                            openDeposit(bestCopilotAsset);
                          }}
                        >
                          <CircleArrowDown data-icon="inline-start" />
                    ...
                    {copilotResult.items.map((item) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <span
                            className={`inline-flex shrink-0 whitespace-nowrap rounded-md border px-2 py-0.5 text-[10px] font-semibold ${item.status === "actionable" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : item.status === "hold" ? "border-sky-500/30 bg-sky-500/10 text-sky-300" : item.status === "blocked" ? "border-amber-500/30 bg-amber-500/10 text-amber-300" : "border-red-500/30 bg-red-500/10 text-red-300"}`}
                          >
                            {item.label}
                          </span>
                        </div>
                        ...
                        {item.factors.length > 0 || item.risks.length > 0 ? (
                          <details className="group mt-3 max-w-full">
                            ...
                            <div className="mt-3 space-y-4 rounded-xl border border-border/50 bg-background/25 p-3">
